...
**maths-v2 (§12) is the standard for every maths book** (27 September 2026).
Where an earlier section describes the house design or `maths-clear` — the
three working colours of §1, the furniture, a single chapter palette — it
describes what unconverted chapters still carry; §12 and
[CONVERT-V2.md](CONVERT-V2.md) say what a maths chapter is now.

---

## 1. Colour — three working colours, used semantically

*The house design's colour. A maths-v2 chapter — the standard — takes a
structure colour of its own and fixed box colours instead; see §12.*
...
## 11. `maths-clear` — the base layer under maths-v2
...
## 12. maths-v2 — the standard for every maths book

**maths-v2 is the maths design** (made the standard on 27 September 2026).
Every maths chapter is to be on it; the house design and `maths-clear` are
the designs the unconverted chapters still carry, not alternatives. A
chapter declares:
...
It loads `maths-clear.css` and then `css/maths-v2.css`, so maths-clear is its
base layer, not a rival. Class 6 (all ten chapters) is on it; Classes 7–10 are
converted with [CONVERT-V2.md](CONVERT-V2.md), which is the recipe, with Class 6
Chapter 1 as the model. A volume will not bind while its chapters are on two
designs or two editions, so a class moves over whole.

### The chapter

Three parts: the body (NCERT's syllabus in NCERT's order, in our own words,
ending on a Summary page), **By the Book** (`p090`–, board forms, see
[BY-THE-BOOK.md](BY-THE-BOOK.md)) and **Beyond the Book** (`p101`–,
competitive formats, then the Answers). Every section of the body carries a
key idea and a Think and Reflect. Nothing of NCERT's expression is
reproduced — wording, stories, characters, data, figures or question text;
NCERT's exercises are cited by page and number. See CONVERT-V2.md.

### Colour

- **Each chapter has its own structure colour** — the `--teal` family in
  `css/palette-prism-<name>.css`: the head and foot tabs, the opener, section
  heads, tables, the running title and folio. All of them sit at one OKLCH
  lightness and one low chroma (L 0.46, C 0.07) and only the hue moves, so
  none is louder than another and each is quieter than the boxes. Class 6 is
  indigo, rose, petrol, plum, steel, claret, dusk, mauve, slate and heather.
  A new class computes its own set the same way; hand-picked vivid colours
  were tried and rejected.
- **Each box keeps one colour in every chapter**, so a colour keeps its
  meaning: sky worked examples, sunflower Think and Reflect, berry key ideas,
  violet summary, and exercise sets and By the Book in Class 7's muted mauve
  (`#915074` on `#faedf3`).
- No green, no brown. `.dg-fill-d` prints beige and is not used.

This replaces §1's three working colours for maths-v2 chapters.

### Components

- **Key idea** as a printed definition box: a double rule all round, the title
  set into the top rule.
- **Exercise sets** as one tinted field under a mauve band; a set may continue
  overleaf under `.c-practice--cont`. A figure belongs inside its question's
  `<li>`.
- **Tables** with a solid structure-colour head row; a grid of raw data with no
  row names takes `.table--raw`.
- **One radius, 2.4mm, on every box.** No side-bar callouts, pills or shadows.
- A caption inside a question is centred like every other.

### Furniture

Taken from the science-v2 pages: a chapter tab bleeding off the head's left
corner with a slanted end and a paler slant behind it, holding a small
+ − × ÷ motif and CHAPTER N; the running title at the right over a hairline
(verso the chapter title, recto the section in force, or By the Book / Beyond
the Book); at the foot, *ClassBridge · Mathematics N* over a hairline and the
folio in a matching slanted tab at the outer corner. The builder writes it
(`v2Runhead`, `v2Pagefoot`); `maths-v2.css` §13 sizes it. The full-width
indigo bands, thumb tabs and a gradient wedge were tried and rejected.

### Checks

Beyond the builder: `lone-words`, `check-sums`, `orphans`, `check-labels`,
`fit-options` and the chapter's own `check-numbers.mjs` must all pass; every
page is at least 88% full except the Summary, By the Book's last page and the
chapter's last page.
