...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch06-content.mjs';
import {adolescenceDiagram} from './science-g7-ch06-diagrams.mjs';
...
const dir='pages/class-7/ch06-adolescence',history='assets/design-history/science-g7-ch06';
...
const motifs=(running=false)=>`<g class="${running?'v2-change-motif':'chapter-opener__motif v2-change-motif'}" transform="${running?'translate(26 9) scale(.40)':'translate(951 84) scale(1.05)'}"><path d="M32 95V35C10 36 0 21 2 7C22 5 33 19 32 35C32 13 47 0 66 0C66 23 50 36 32 35M13 95H52"/></g>`;
...
const probe=path.resolve('build/_g7-ch06-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=adolescenceDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  ...
  if(meta.diagram){const d=adolescenceDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
const band=chapterOpener({id:'science-g7-ch06',number,titleLines:['Adolescence: A Stage','of Growth and Change'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch06/opener.png`,aspect:1.5,alt:'Young people observing stages of plant growth'}});
