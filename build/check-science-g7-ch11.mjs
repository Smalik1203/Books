...
import {opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch11-content.mjs';
import {scienceContract} from './science-contract.mjs';
const chapter='class-7/ch11-light-shadows-reflections',dir='pages/'+chapter,history='assets/design-history/science-g7-ch11';
...
assert.deepEqual(lesson.filter(b=>b.sourceActivity).map(b=>b.sourceActivity),Array.from({length:10},(_,i)=>'11.'+(i+1)));
assert.equal((all.match(/data-feature="setup"/g)||[]).length,10);
assert.equal((all.match(/data-feature="think"/g)||[]).length,3);
assert.equal(exercises.length,12);assert.equal(projects.length,4);assert.equal(glossary.length,14);assert.equal(summary.length,10);
for(const phrase of ['at this thickness','roughness spreads it','front and back','diffraction eventually limits sharpness','Never aim at or look towards the Sun','never use broken bangles or glass','not supplied answers','Both openings face left'])has(phrase);
for(const [task,result] of [['holes-task','holes-evidence'],['tube-task','tube-evidence'],['materials-task','transmission-table'],['shadow-task','shadow-explanation'],['redirect-task','reflection-definition'],['slit-task','reflection-evidence'],['pen-task','virtual-image'],['self-task','lateral'],['pinhole-task','pinhole-explanation'],['camera-task','camera-distance']])assert.ok(lesson.findIndex(b=>b.id===task)<lesson.findIndex(b=>b.id===result),'Observation before explanation: '+task);
assert.equal(lesson.filter(b=>b.comparisonTable).length,4);
assert.ok(!lesson.some(b=>b.type==='comparison'));
for(const q of exercises)if(q.table){[q.table.caption,q.table.note,...q.table.rows.flat()].forEach(has);assert.ok(pages.some(p=>[q.text,q.table.caption,q.table.note,...q.table.rows.flat()].every(s=>p.includes(norm(s)))),'Whole matching question: '+q.id);}
...
assert.equal(coverage.points.length,39);
...
const assets=JSON.parse(await fs.readFile(history+'/artwork.json','utf8'));assert.equal(assets.length,13);
...
for(const m of all.matchAll(/<image\b[^>]*href="([^"]+)"/g)){assert.ok(m[1].endsWith('.png')&&m[1].includes('/class-7/science/ch11/'));await fs.access(path.resolve('build/class-7',m[1]));}
...
console.log(`Chapter 11 checks passed: ${files.length} pages, 10 source activities, 12 questions, 4 projects, 13 transparent assets. Actual mean lesson occupancy ${mean.toFixed(1)}%; short-page exceptions recorded.`);
