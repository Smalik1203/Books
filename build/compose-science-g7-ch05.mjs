...
// Independent Grade 7 Chapter 5 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch05-content.mjs';
import {changeDiagram} from './science-g7-ch05-diagrams.mjs';
...
const dir='pages/class-7/ch05-physical-chemical-changes',history='assets/design-history/science-g7-ch05';
...
const motifs=(running=false)=>`<g class="${running?'v2-change-motif':'chapter-opener__motif v2-change-motif'}" transform="${running?'translate(26 9) scale(.40)':'translate(951 84) scale(1.05)'}"><path d="M17 0H45M22 0V34L2 78Q-5 92 12 92H55Q71 92 64 78L40 34V0 M13 63H52 M24 72H25 M39 82H40"/></g>`;
...
const probe=path.resolve('build/_g7-ch05-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='diagram'){
  const d=changeDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=changeDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  ...
  if(meta.diagram){const d=changeDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
const band=chapterOpener({id:'science-g7-ch05',number,titleLines:['Changes Around Us:','Physical and Chemical'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch05/opener.png`,aspect:1.5,alt:'Children comparing melting ice, ripening bananas and a flowering rose'}});
...
pages.push({title:'Keywords',titleRole:'reference',source:[12],parts:ref,end:refBottom});
...
pages.push({title:'Summary',titleRole:'reference',source:[12],parts:sum,end:sp.h});
blocks.length=0;
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[13]});
...
for(const p of projects){render({id:p.id+'-head',type:'heading',level:2,text:p.title,source:p.source});render({id:p.id,type:'body',text:p.text,source:p.source,keepNext:!!p.figure,keepWhole:!!p.figure});if(p.figure)render({id:p.id+'-figure',type:'figure',figure:p.figure,caption:p.caption,source:p.source});}
