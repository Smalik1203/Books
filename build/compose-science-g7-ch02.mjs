...
// Independent Grade 7 Chapter 2 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch02-content.mjs';
...
const dir='pages/class-7/ch02-exploring-substances',history='assets/design-history/science-g7-ch02';
...
const motifs=(running=false)=>`<g class="${running?'v2-drop-motif':'chapter-opener__motif v2-drop-motif'}" transform="${running?'translate(26 7) scale(.36)':'translate(983 73) scale(.48)'}"><path d="M28 8C22 24 4 39 4 55a24 24 0 0 0 48 0C52 39 34 24 28 8Z M17 54a11 11 0 0 0 11 11"/><path d="M56 92h33M63 85v39a14 14 0 0 0 28 0V85M64 111h26"/></g>`;
...
const probe=path.resolve('build/_g7-ch02-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='comparison'){
  ...
  for(const [i,col] of meta.columns.entries()){
   const x=89+i*300,w=274,head=wrap(col.title,w,'h');let top=head.length*36+14;
...
const band=chapterOpener({id:'science-g7-ch02',number,titleLines:['Exploring Substances:','Acidic, Basic, and Neutral'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch02/opener.png`,aspect:1.5,alt:'Students and a teacher exploring natural indicators'}});
const motif=`<g transform="translate(8 0)">${scienceHeaderArt(7,2)}</g>`;
...
pages.push({title:'Keywords',titleRole:'reference',source:[4,6,10,12],parts:ref,end:refBottom});
...
blocks.length=0;
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[14]});
...
for(const p of projects){render({id:p.id+'-head',type:'heading',level:2,text:p.title,source:p.source});render({id:p.id,type:'body',text:p.text,source:p.source});}
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g7-ch02${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 2 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
...
console.log(`Chapter 2: ${pages.length} pages; ${fitted.length} lesson pages; ${lessonBlockCount} protected/prose blocks.\n`+map.map(p=>`p${p.page}: ${p.fill}% ${p.title||'continued lesson'}`).join('\n'));
