...
// Independent Grade 6 Chapter 8 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,games,exercises,projects} from './science-g6-ch08-content.mjs';
import {waterDiagram} from './science-g6-ch08-diagrams.mjs';
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M50 5C43 24 15 45 15 65A35 30 0 0 0 85 65C85 45 57 24 50 5ZM29 61Q23 77 41 82"/></g>`;
...
const labels='Keywords Summary Let Us Enhance Our Learning Use your notebook. Support each answer with evidence and explain any uncertainty. Explore Further Let Us Play';
...
const probe=path.resolve('build/_g6-ch08-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='project'){
  ...
  if(meta.diagram){const d=waterDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
  return add(meta,html,y+12,{conceptId:meta.id});
 }

 if(meta.type==='diagram'){
  const d=waterDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=waterDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  if(meta.layout==='side'){
   ...
   const diagram=meta.diagram?waterDiagram(meta.diagram):{html:photo(meta.figure,650,0,290,meta.figureHeight||270),h:meta.figureHeight||270};
 ...
 let html='',y=0;
 if(meta.figure){
  const height=({condensation:330,evaporation:245,cooler:430,cycle:450,balance:385})[meta.figure]||270;
...
const band=chapterOpener({id:'science-g6-ch08',number,titleLines:['A Journey through','States of Water'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch08/opener.png`,aspect:1.8,alt:'Aavi, Thirav and an adult studying ice in lemonade'}});
...
blocks.length=0;
if(games.length)render({id:'games-heading',type:'heading',text:'Let Us Play',source:[18,19]});
for(const game of games)render({...game,type:'project'});
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[19,20]});
...
blocks.length=0;
render({id:'project-heading',type:'heading',text:'Explore Further',source:[20]});
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g6-ch08${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 8 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
