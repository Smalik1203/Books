...
// Independent Grade 7 Chapter 3 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch03-content.mjs';
import {circuitDiagram} from './science-g7-ch03-diagrams.mjs';
...
const dir='pages/class-7/ch03-electricity-circuits',history='assets/design-history/science-g7-ch03';
...
const motifs=(running=false)=>`<g class="${running?'v2-electric-motif':'chapter-opener__motif v2-electric-motif'}" transform="${running?'translate(26 5) scale(.55)':'translate(951 72) scale(1.1)'}"><path d="M0 35H20M20 10V60M34 22V48M34 35H60V85H42M18 85H0V35"/><circle cx="30" cy="85" r="12"/><path d="M22 77l16 16M22 93l16 -16"/></g>`;
...
const probe=path.resolve('build/_g7-ch03-type-measure.html');
...
function body(meta){const rows=wrap(meta.text),chunks=[];for(let i=0;i<rows.length;){const n=rows.length-i>3?2:rows.length-i;chunks.push(rows.slice(i,i+n));i+=n;}chunks.forEach((rows,i)=>add({...meta,id:meta.id+'-'+i},lines(rows,89,0),rows.length*32+(i===chunks.length-1?16:0),{paragraphPart:i,paragraphParts:chunks.length,keepNext:!!meta.keepNext&&i===chunks.length-1}));}
...
function render(meta){
 ...
 if(meta.type==='diagram'){
  const d=circuitDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='question'){
  ...
  if(meta.diagram){const d=circuitDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
  if(meta.figure){html+=photo(meta.figure,222,y,680,215);y+=220;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
const band=chapterOpener({id:'science-g7-ch03',number,titleLines:['Electricity: Circuits','and their Components'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch03/opener.png`,aspect:1.5,alt:'Students and their teacher exploring a battery torch'}});
const motif=scienceHeaderArt(7,3);
...
pages.push({title:'Keywords',titleRole:'reference',source:[3,4,5,8,9,10,14],parts:ref,end:refBottom});
...
blocks.length=0;
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[15]});
...
blocks.length=0;
render({id:'project-heading',type:'heading',text:'Explore Further',source:[17,18]});
for(const p of projects){render({id:p.id+'-head',type:'heading',level:2,text:p.title,source:p.source});render({id:p.id,type:'body',text:p.text,source:p.source,keepNext:!!p.figure});if(p.figure)render({id:p.id+'-figure',type:'figure',figure:p.figure,caption:p.caption,source:p.source});}
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g7-ch03${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 3 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
...
console.log(`Chapter 3: ${pages.length} pages; ${fitted.length} lesson pages; ${lessonBlockCount} protected/prose blocks.\n`+map.map(p=>`p${p.page}: ${p.fill}% ${p.title||'continued lesson'}`).join('\n'));
