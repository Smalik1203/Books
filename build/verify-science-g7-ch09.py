...
chapter='class-7/ch09-life-processes-in-animals'
history=root/'assets/design-history/science-g7-ch09'
review=root/'build/_class7-ch09-review'
...
for folder,pattern in [(review,'reading-*.png'),(root/'build'/f'{chapter}-proofs','p*.png')]:
    ...
    for old in folder.glob(pattern):
        ...
        if number and int(number.group(1))>23: old.unlink()
...
print(f'PDFs verified: 23 pages each; {len(assets)} native transparent image resources; {len(before)} existing files unchanged; Science studio registration confirmed.')
