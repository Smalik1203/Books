...
// Independent Grade 7 Chapter 4 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch04-content.mjs';
import {metalDiagram} from './science-g7-ch04-diagrams.mjs';
...
const dir='pages/class-7/ch04-metals-non-metals',history='assets/design-history/science-g7-ch04';
...
const motifs=(running=false)=>`<g class="${running?'v2-metal-motif':'chapter-opener__motif v2-metal-motif'}" transform="${running?'translate(26 9) scale(.40)':'translate(951 84) scale(1.05)'}"><path d="M0 14L39 0L66 18L28 33Z M0 14V39L28 58L66 43V18 M28 33V58 M8 71C8 57 58 57 58 71S8 85 8 99S58 113 58 99"/></g>`;
...
const probe=path.resolve('build/_g7-ch04-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='panel'&&meta.figure){
  render({id:meta.id+'-equipment',type:'figure',figure:meta.figure,caption:meta.figureCaption,source:meta.source});
  meta={...meta,figure:undefined};
 }
 if(meta.type==='diagram'){
  const d=metalDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='question'){
  ...
  if(meta.diagram){const d=metalDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
const band=chapterOpener({id:'science-g7-ch04',number,titleLines:['The World of Metals','and Non-metals'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch04/opener.png`,aspect:1.5,alt:'Students observing an ironsmith with their grandfather'}});
const motif=scienceHeaderArt(7,4);
...
pages.push({title:'Keywords',titleRole:'reference',source:[13],parts:ref,end:refBottom});
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g7-ch04${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 4 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
...
console.log(`Chapter 4: ${pages.length} pages; ${fitted.length} lesson pages; ${lessonBlockCount} protected/prose blocks.\n`+map.map(p=>`p${p.page}: ${p.fill}% ${p.title||'continued lesson'}`).join('\n'));
