...
// Independent Grade 6 Chapter 9 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,games,exercises,projects} from './science-g6-ch09-content.mjs';
...
const probe=path.resolve('build/_g6-ch09-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='panel'){
  ...
  if(meta.figure){const ah=meta.figure==='filtration'?360:235;html+=photo(meta.figure,176,y,700,ah);y+=ah+10;const rows=wrap(meta.figureCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 let html='',y=0;
 if(meta.figure){
  const height=({winnowing:350,sieve:270,salt:235,filtration:360,churning:305,magnet:290})[meta.figure]||270;
...
const band=chapterOpener({id:'science-g6-ch09',number,titleLines:['Methods of Separation','in Everyday Life'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch09/opener.png`,aspect:1.8,alt:'Malli and Valli examining grains with their grandmother'}});
...
pages.push({title:'Keywords',titleRole:'reference',source:[14],parts:ref,end:refBottom});
...
pages.push({title:'Summary',titleRole:'reference',source:[15],parts:sum,end:sp.h});
blocks.length=0;
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g6-ch09${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 9 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
...
console.log(`Chapter 9: ${pages.length} pages; ${fitted.length} lesson pages; ${lessonBlockCount} protected/prose blocks.\n`+map.map(p=>`p${p.page}: ${p.fill}% ${p.title||'continued lesson'}`).join('\n'));
