...
// Independent Grade 6 Chapter 7 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,games,exercises,projects} from './science-g6-ch07-content.mjs';
import {temperatureDiagram} from './science-g6-ch07-diagrams.mjs';
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M40 64V17Q40 4 53 4Q66 4 66 17V64A21 21 0 1 1 40 64ZM53 29V80M72 20H85M72 35H80M72 50H85"/></g>`;
...
const probe=path.resolve('build/_g6-ch07-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='project'){
  ...
  if(meta.diagram){const d=temperatureDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
  return add(meta,html,y+12,{conceptId:meta.id});
 }

 if(meta.type==='diagram'){
  const d=temperatureDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=temperatureDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  if(meta.layout==='side'){
   ...
   const diagram=meta.diagram?temperatureDiagram(meta.diagram):{html:photo(meta.figure,650,0,290,meta.figureHeight||270),h:meta.figureHeight||270};
 ...
 let html='',y=0;
 if(meta.figure){
  const height=({bowls:270,clinical:300,weather:345})[meta.figure]||270;
  html=photo(meta.figure,89,0,874,height);y=height+10;
  const figureLabels={bowls:[['A · warm',235],['B · room temperature',526],['C · cool',817]]}[meta.figure];
...
const band=chapterOpener({id:'science-g6-ch07',number,titleLines:['Temperature and','its Measurement'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch07/opener.png`,aspect:1.8,alt:'Teacher and students examining a digital thermometer'}});
...
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[15,16,17,18]});
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g6-ch07${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 7 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
