...
// Independent Grade 7 Chapter 11 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch11-content.mjs';
import {lightDiagram} from './science-g7-ch11-diagrams.mjs';
...
const dir='pages/class-7/ch11-light-shadows-reflections',history='assets/design-history/science-g7-ch11';
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M15 98L47 44L86 96M47 44V6M47 44L10 21M47 44L89 20M14 104H88M25 112H78"/><circle cx="47" cy="44" r="6"/></g>`;
...
const probe=path.resolve('build/_g7-ch11-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='diagram'){
  const d=lightDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=lightDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  if(meta.layout==='side'){
   ...
   const diagram=meta.diagram?lightDiagram(meta.diagram):{html:photo(meta.figure,650,0,290,meta.figureHeight||270),h:meta.figureHeight||270};
  ...
  if(meta.diagram){const d=lightDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
const band=chapterOpener({id:'science-g7-ch11',number,titleLines:['Light: Shadows','and Reflections'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch11/opener.png`,aspect:1.5,alt:'Children and an elder observing fireflies'}});
...
pages.push({title:'Keywords',titleRole:'reference',source:[2,3,5,6,8,9,10,11,12],parts:ref,end:refBottom});
...
pages.push({title:'Summary',titleRole:'reference',source:[13],parts:sum,end:sp.h});
blocks.length=0;
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[13,14,15]});
