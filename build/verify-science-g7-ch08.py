...
chapter='class-7/ch08-time-and-motion'
history=root/'assets/design-history/science-g7-ch08'
review=root/'build/_class7-ch08-review'
...
for folder,pattern in [(review,'reading-*.png'),(root/'build'/f'{chapter}-proofs','p*.png')]:
    ...
    for old in folder.glob(pattern):
        ...
        if number and int(number.group(1))>20: old.unlink()
...
print(f'PDFs verified: 20 pages each; {len(assets)} native transparent image resources; {len(before)} existing files unchanged; Science studio registration confirmed.')
