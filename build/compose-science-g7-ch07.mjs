...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch07-content.mjs';
import {heatDiagram} from './science-g7-ch07-diagrams.mjs';
...
const dir='pages/class-7/ch07-heat-transfer-in-nature',history='assets/design-history/science-g7-ch07';
...
const motifs=(running=false)=>`<g class="${running?'v2-change-motif':'chapter-opener__motif v2-change-motif'}" transform="${running?'translate(26 9) scale(.40)':'translate(951 84) scale(1.05)'}"><path d="M5 90C-8 66 27 60 14 36M30 104C13 75 54 54 38 19M56 94C38 66 78 47 62 10"/></g>`;
...
const probe=path.resolve('build/_g7-ch07-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='diagram'){
  const d=heatDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=heatDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  ...
  if(meta.diagram){const d=heatDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
pages.push({title:'Keywords',titleRole:'reference',source:[13,14],parts:ref,end:refBottom});
...
pages.push({title:'Summary',titleRole:'reference',source:[13,14],parts:sum,end:sp.h});
blocks.length=0;
