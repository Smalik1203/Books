...
// Independent Grade 7 Chapter 9 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,exercises,projects} from './science-g7-ch09-content.mjs';
import {lifeDiagram} from './science-g7-ch09-diagrams.mjs';
...
const dir='pages/class-7/ch09-life-processes-in-animals',history='assets/design-history/science-g7-ch09';
...
const motifs=(running=false)=>`<g class="${running?'v2-change-motif':'chapter-opener__motif v2-change-motif'}" transform="${running?'translate(26 9) scale(.40)':'translate(951 84) scale(1.05)'}"><path d="M38 9V40M38 28L17 48M38 28L59 48"/><circle cx="15" cy="65" r="18"/><circle cx="61" cy="65" r="18"/><circle cx="38" cy="94" r="17"/><path d="M21 81L28 84M56 81L49 84"/></g>`;
...
const probe=path.resolve('build/_g7-ch09-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='project'){
  const rows=wrap(meta.title,874,'h'),y=rows.length*36+12,p=paragraph(meta.text,{y});
  return add(meta,lines(rows,89,0,'se-heading',36,30)+p.html,y+p.h+12,{conceptId:meta.id});
 }

 if(meta.type==='diagram'){
  const d=lifeDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=lifeDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  ...
  if(meta.diagram){const d=lifeDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
...
const band=chapterOpener({id:'science-g7-ch09',number,titleLines:['Life Processes','in Animals'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch09/opener.png`,aspect:1.5,alt:'Children eating lunch and resting after play in a school garden'}});
