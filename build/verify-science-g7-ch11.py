...
chapter='class-7/ch11-light-shadows-reflections'
history=root/'assets/design-history/science-g7-ch11'
review=root/'build/_class7-ch11-review'
    ...
    for p in pdf.pages:
        ...
        for ref in r.get('/XObject',{}).values():
            ...
            if o.get('/Subtype')=='/Image':
                ...
                masks+=1;native+=1
    assert masks>=len(assets),(suffix,masks,len(assets))
    assert any('SourceSerif4' in f for f in fonts) and any('SourceSans3' in f for f in fonts)
    report['exports'].append({'file':str(file.relative_to(root)),'pages':24,'mediaMM':[round(n,2) for n in size],'transparentImages':masks,'fonts':sorted(fonts),'sha256':sha(file)})
...
for folder,pattern in [(review,'reading-*.png'),(root/'build'/f'{chapter}-proofs','p*.png')]:
    ...
    for old in folder.glob(pattern):
        ...
        if number and int(number.group(1))>24: old.unlink()
...
print(f'PDFs verified: 24 pages each; {len(assets)} native transparent image resources; {len(before)} existing files unchanged; Science studio registration confirmed.')
