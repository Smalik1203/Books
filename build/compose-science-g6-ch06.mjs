...
// Independent Grade 6 Chapter 6 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,games,exercises,projects} from './science-g6-ch06-content.mjs';
import {materialDiagram} from './science-g6-ch06-diagrams.mjs';
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M12 28L50 8L87 28L50 48ZM12 28V75L50 97L87 75V28M50 48V97"/></g>`;
...
const probe=path.resolve('build/_g6-ch06-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='project'){
  ...
  if(meta.diagram){const d=materialDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const c=caption(meta.caption,y);html+=c.html;y+=c.h+12;}
  return add(meta,html,y+12,{conceptId:meta.id});
 }

 if(meta.type==='diagram'){
  const d=materialDiagram(meta.diagram),c=caption(meta.caption,d.h+10);
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=materialDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+12;const rows=wrap(meta.diagramCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  if(meta.layout==='side'){
   ...
   const diagram=meta.diagram?materialDiagram(meta.diagram):{html:photo(meta.figure,650,0,290,meta.figureHeight||270),h:meta.figureHeight||270};
 ...
 let html='',y=0;
 if(meta.figure){
  const height=({mixing:285,panes:300,balls:255,hardness:235,pottery:300})[meta.figure]||270;
  html=photo(meta.figure,89,0,874,height);y=height+10;
  const figureLabels={
   mixing:[['Sugar',190],['Salt',344],['Chalk powder',499],['Sand',653],['Sawdust',806]],
   balls:[['Tennis ball',284],['Cricket ball',525],['Hand-exercise ball',768]],
   panes:[['Clear glass',243],['Frosted glass',526],['Wood',809]],
   hardness:[['Sponge',278],['Eraser',450],['Wood',622],['Steel spoon',805]]
  }[meta.figure];
...
const band=chapterOpener({id:'science-g6-ch06',number,titleLines:['Materials','Around Us'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch06/opener.png`,aspect:1.5,alt:'Teacher and students comparing everyday objects and their materials'}});
...
pages.push({title:'Keywords',titleRole:'reference',source:[17],parts:ref,end:refBottom});
...
blocks.length=0;
render({id:'games-heading',type:'heading',text:'Let Us Play',source:[18,19]});
...
blocks.length=0;
render({id:'project-heading',type:'heading',text:'Explore Further',source:[21]});
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g6-ch06${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 6 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
...
console.log(`Chapter 6: ${pages.length} pages; ${fitted.length} lesson pages; ${lessonBlockCount} protected/prose blocks.\n`+map.map(p=>`p${p.page}: ${p.fill}% ${p.title||'continued lesson'}`).join('\n'));
