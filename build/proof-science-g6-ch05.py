"""Render all Chapter 5 pages and verify print geometry and embedded images."""
from pathlib import Path
from pypdf import PdfReader
from PIL import Image,ImageDraw
import json,subprocess,hashlib
root=Path(__file__).resolve().parent.parent
chapter='class-6/ch05-measurement-length-motion'
history=root/'assets/design-history/science-g6-ch05'
review=root/'build/_class6-ch05-review';review.mkdir(exist_ok=True)
report={'chapter':chapter,'assets':[],'exports':[]}
page_count=len(list((root/'pages'/chapter).glob('p[0-9][0-9][0-9].html')))
for a in json.loads((history/'artwork.json').read_text()):
...
proofs=sorted(review.glob('page-*.png'))
for batch in range((page_count+4)//5):
...
print(f'{page_count} pages rendered; transparent assets, live text, trim and bleed dimensions verified.')
