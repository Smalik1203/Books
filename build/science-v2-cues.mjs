...
export function v2PanelHeading(kind, left, baseline) {
 ...
 return `<g class="v2-panel-heading v2-panel-heading--${kind}" data-feature="${kind}" aria-label="${cue.title}"><svg class="v2-panel-icon" x="${left-4}" y="${baseline-22}" width="32" height="32" viewBox="0 0 24 24" aria-hidden="true"><path d="${cue.paths}"/></svg><text class="v2-panel-title ${cue.titleClass}" x="${left+40}" y="${baseline}">${cue.title}</text></g>`;
}
