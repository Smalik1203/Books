import {pageVisual} from './science-g7-ch12-diagrams.mjs';
...
export function refitV2Lesson(plans){
 ...
 for(let start=n-1;start>=0;start--){
  for(let end=start+1;end<=n&&prefix[end]-prefix[start]-blocks[start].before<=capacity;end++){
   ...
   const illustrated=blocks.slice(start,end).some(b=>b.html.includes('<image '));
   const extra=illustrated?0:212;
   const used=prefix[end]-prefix[start]-blocks[start].before+extra,gap=capacity-used;
   if(gap<0)continue;
 ...
 for(let start=0;start<n;){
  ...
  const blockAudit=[];
  let parts=selected.map((b,i)=>{const before=i?b.before:0,h=b.h-b.before; y+=before;const dy=y-b.top;blockAudit.push({id:b.id,type:b.type,text:b.text,artKey:b.artKey,top:y,bottom:y+h,before,keepNext:!!b.keepNext,comparisonItems:b.comparisonItems,conceptId:b.conceptId});y+=h;return `<g class="v2-reading-block" data-block="${b.id}" transform="translate(0 ${dy})">${b.html}</g>`;}).join('\n');
  if(!parts.includes('<image ')){const v=pageVisual(selected.flatMap(b=>b.source));parts+=`<g transform="translate(0 ${y+20})">${v.html}</g>`;y+=v.h+20;}
