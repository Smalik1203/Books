...
const run=promisify(execFile),chapter=process.argv[2]||'class-6/ch05-measurement-length-motion';
const history=process.argv[3]||'assets/design-history/science-g6-ch05';
...
onload=async()=>{
 ...
 for(const svg of document.querySelectorAll('.science-sheet')){
  ...
  if(band){
   ...
   opener={bandBounds:box(band),textBounds:ink,bleedCovered:[-bleed,width/2,width+bleed].every(x=>[-bleed,0,266].every(y=>band.isPointInFill(new DOMPoint(x,y)))),safeText:ink.every(b=>b.left>=89&&b.right<=963&&b.top>=30&&b.bottom<=270),alignedBaseline:number.getAttribute('y')===titles.at(-1).getAttribute('y'),centredLabel:label.getAttribute('x')===number.getAttribute('x'),numberTitleGap:box(titles[0]).left-box(number).right,decorationGap:box(svg.querySelector('[data-header-art]')).left-Math.max(...titles.map(e=>box(e).right))};
   const garden=box(svg.querySelector('image.science-illustration'));
   opener.headerImageGap=garden.top-Math.max(box(band).bottom,box(svg.querySelector('[data-header-art]')).bottom);
