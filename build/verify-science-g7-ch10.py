...
chapter='class-7/ch10-life-processes-in-plants'
history=root/'assets/design-history/science-g7-ch10'
review=root/'build/_class7-ch10-review'
...
for a in assets:
    ...
    alpha=sum(im.getchannel('A').histogram()[:3])/(im.width*im.height)
    assert alpha>.1 and max(im.size)>=1024
    ...
    for p in pdf.pages:
        ...
        for ref in r.get('/XObject',{}).values():
            ...
            if o.get('/Subtype')=='/Image':
                ...
                assert max(o['/Width'],o['/Height'])>=1024
                masks+=1;native+=1
    assert masks==len(assets)+1,(suffix,masks,len(assets))
    assert any('SourceSerif4' in f for f in fonts) and any('SourceSans3' in f for f in fonts)
    report['exports'].append({'file':str(file.relative_to(root)),'pages':25,'mediaMM':[round(n,2) for n in size],'transparentImages':masks,'fonts':sorted(fonts),'sha256':sha(file)})
...
# Remove only numbered proofs beyond the verified extent, inside this chapter's build paths.
for folder,pattern in [(review,'reading-*.png'),(root/'build'/f'{chapter}-proofs','p*.png')]:
    assert folder.resolve().is_relative_to((root/'build').resolve())
    for old in folder.glob(pattern):
        number=re.search(r'(\d+)\.png$',old.name)
        if number and int(number.group(1))>25: old.unlink()
subprocess.run([r'C:/Users/ragha/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/Library/bin/pdftoppm.exe','-png','-r','110',str(root/'build'/f'{chapter}.pdf'),str(review/'reading')],check=True,capture_output=True)
subprocess.run([r'C:/Users/ragha/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/Library/bin/pdftoppm.exe','-f','1','-l','1','-png','-r','150',str(root/'build'/f'{chapter}-bleed.pdf'),str(review/'bleed')],check=True,capture_output=True)
...
print(f'PDFs verified: 25 pages each; {len(assets)} native transparent image resources; {len(before)} existing files unchanged; Science studio registration confirmed.')
