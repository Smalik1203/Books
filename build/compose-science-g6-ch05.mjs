...
// Independent Grade 6 Chapter 5 authoring entry point. Shared Science typography,
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M9 28H91V77H9ZM22 28V49M36 28V41M50 28V49M64 28V41M78 28V49"/></g>`;
...
function render(meta){
 ...
 if(meta.type==='question'){
  if(meta.layout==='side'){
   ...
   const diagram=meta.diagram?measureDiagram(meta.diagram):{html:photo(meta.figure,650,0,290,meta.figureHeight||270),h:meta.figureHeight||270};
 ...
 let html='',y=0;
 if(meta.figure){const height=meta.figure==='park'?380:270;html=photo(meta.figure,176,0,700,height);y=height+10;}
...
ref+=photo('tape',346,refBottom+35,360,240);refBottom+=295;
...
let sum=label('Summary',89,150,'v2-title');const sp=bulletList(summary,89,874,180);sum+=sp.html;sum+=photo('park',226,sp.h+35,600,330);sp.h+=385;
