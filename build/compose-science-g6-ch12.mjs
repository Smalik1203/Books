...
import {chapterDiagram} from './science-g6-ch12-diagrams.mjs';
import {roomyComparisonTableBlock as comparisonTableBlock} from './science-g7-comparison-tables.mjs';
// Independent Grade 6 Chapter 12 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,games,exercises,projects,closing} from './science-g6-ch12-content.mjs';
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M65 8A40 40 0 1 0 89 72A36 36 0 0 1 65 8ZM82 17V31M75 24H89"/></g>`;
...
const terms=new Set([' ','—']);for(const w of tokens(strings({title,opener,lesson,glossary,summary,games,exercises,projects,closing}).join(' ')+' '+labels))for(const p of w)terms.add(p.s);
const probe=path.resolve('build/_g6-ch12-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='panel'){
  ...
  if(meta.diagram){const d=chapterDiagram(meta.diagram);html+=`<g transform="translate(0 ${y})"><g data-instructional-figure="${meta.diagram}">${d.html}</g></g>`;y+=d.h+10;const r=wrap(meta.diagramCaption,826,'n',.87);html+=lines(r,113,y,'se-caption',27,20.88);y+=r.length*27+12;}
...
const band=chapterOpener({id:'science-g6-ch12',number,titleLines:['Beyond','Earth'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch12/opener.png`,aspect:1.8,alt:'Yangdol and Dorjay watching the sky with an adult in Ladakh'}});
...
pages.push({title:'Summary',titleRole:'reference',source:[18,19],parts:sum,end:sp.h});
blocks.length=0;
...
render({id:'closing-heading',type:'heading',text:closing.title,source:closing.source});
for(const [i,text] of closing.paragraphs.entries())render({id:'closing-'+i,type:'body',text,source:closing.source,keepWhole:true});
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g6-ch12${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 12 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
