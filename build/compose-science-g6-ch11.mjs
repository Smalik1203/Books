...
import {chapterDiagram} from './science-g6-ch11-diagrams.mjs';
import {roomyComparisonTableBlock as comparisonTableBlock} from './science-g7-comparison-tables.mjs';
// Independent Grade 6 Chapter 11 authoring entry point. Shared Science typography,
...
import {title,shortTitle,opener,lesson,glossary,summary,games,exercises,projects} from './science-g6-ch11-content.mjs';
...
const motifs=(running=false)=>`<g class="${running?'v2-light-motif':'chapter-opener__motif v2-light-motif'}" transform="${running?'translate(22 8) scale(.42)':'translate(946 82) scale(.9)'}"><path d="M30 94Q46 56 61 8M42 65Q17 69 15 52Q33 46 42 65ZM48 47Q28 47 25 33Q42 29 48 47ZM54 30Q36 29 38 15Q52 15 54 30ZM44 60Q70 59 75 41Q54 39 44 60ZM51 43Q76 39 76 24Q56 23 51 43ZM57 25Q73 18 65 4Q55 11 57 25Z"/></g>`;
...
const labels='Keywords Summary Let Us Enhance Our Learning Answer in your notebook, giving reasons. Explore Further Let Us Play';
const strings=value=>typeof value==='string'?[value]:value&&typeof value==='object'?Object.values(value).flatMap(strings):[];
const terms=new Set([' ','—']);for(const w of tokens(strings({title,opener,lesson,glossary,summary,games,exercises,projects}).join(' ')+' '+labels))for(const p of w)terms.add(p.s);
const probe=path.resolve('build/_g6-ch11-type-measure.html');
...
function render(meta){
 ...
 if(meta.type==='illustrated-prose'){
  let y=0,html='';
  for(const s of meta.paragraphs){const p=paragraph(s,{x:89,w:500,y});html+=p.html;y+=p.h;}
  const height=meta.figureHeight||330;html+=photo(meta.figure,613,0,350,height);
  y=Math.max(y,height)+12;const c=caption(meta.caption,y);
  return add(meta,html+c.html,y+c.h+22,{artKey:meta.figure});
 }
 ...
 if(meta.type==='panel'){
  ...
  if(meta.figure){const ah=meta.figureHeight||235;html+=photo(meta.figure,176,y,700,ah);y+=ah+10;const rows=wrap(meta.figureCaption,826,'n',.87);html+=lines(rows,113,y,'se-caption',27,20.88);y+=rows.length*27+12;}
 ...
 if(meta.type==='question'){
  if(meta.layout==='side'){
   ...
   const diagram={html:photo(meta.figure,650,0,290,meta.figureHeight||270),h:meta.figureHeight||270};
 ...
 if(meta.type==='table'){const t=comparisonTableBlock(meta,{wrap,lines});return add(meta,t.html,t.h);}

 let html='',y=0;
 if(meta.figure){
  const height=meta.figureHeight||270;
  html=photo(meta.figure,89,0,874,height);y=height+10;
  const figureLabels=null;
  if(figureLabels){html+=figureLabels.map(([s,x])=>label(s,x,y+24,'se-caption','middle')).join('');y+=42;}
 }
...
const band=chapterOpener({id:'science-g6-ch11',number,titleLines:['Nature’s','Treasures'],bleed:Math.ceil(metrics.bleed*1052/metrics.trimW),image:{href:`../../figures/class-${grade}/science/ch11/opener.png`,aspect:1.8,alt:'Bhoomi, Surya and Ajji observing plants and a stream at the forest edge'}});
...
for(let col=0;col<2;col++){let yy=top+24;for(const [term,meaning] of glossary.slice(col*8,(col+1)*8)){const p=paragraph('**'+term+'** — '+meaning,{x:113+col*428,w:398,y:yy,gap:20});refContents+=p.html;yy+=p.h;}refBottom=Math.max(refBottom,yy+10);}
...
pages.push({title:'Keywords',titleRole:'reference',source:[20],parts:ref,end:refBottom});
let sum=label('Summary',89,150,'v2-title');const sp=bulletList(summary,89,874,180);sum+=sp.html;sum+=photo('review-summary',166,sp.h+35,720,280);sp.h+=335;
if(sp.h>1415)throw Error('Summary overflow');
pages.push({title:'Summary',titleRole:'reference',source:[20],parts:sum,end:sp.h});
blocks.length=0;
if(games.length)render({id:'games-heading',type:'heading',text:'Let Us Play',source:[15,16]});
for(const game of games)render({...game,type:game.type||'project'});
render({id:'assessment-heading',type:'heading',text:'Let Us Enhance Our Learning',source:[21,22,23]});
render({id:'assessment-intro',type:'body',text:'Answer in your notebook, giving reasons.',source:[]});
...
blocks.length=0;
render({id:'project-heading',type:'heading',text:'Explore Further',source:[23,24]});
...
for(let i=0;i<pages.length;i++){
 ...
 const html=`<section class="page page--food page--science-editorial page--science-v2 page--g6-ch11${i===0?' page--opener':''}" data-folio="${n}"${i===pages.length-1?' data-close':''}><div class="page__body"><div class="page__main"><svg class="food-sheet science-sheet science-editorial" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1052 1514" aria-label="${title}, page ${n}">${running}${p.parts}${foot}</svg></div></div></section>`;
 scienceContract('Chapter 11 '+n,html);await writeScienceSource(`${dir}/p${String(n).padStart(3,'0')}.html`,html);
...
await writeScienceSource(history+'/editorial-ledger.json',JSON.stringify({opener:{source:[1],paragraphs:opener},lesson,glossary:{purpose:'Reference definitions for terms taught in the chapter',entries:glossary},summary,games,assessment:{purpose:'All sixteen source questions retained with safety and evidence qualifications',questions:exercises},projects},null,2));
console.log(`Chapter 11: ${pages.length} pages; ${fitted.length} lesson pages; ${lessonBlockCount} protected/prose blocks.\n`+map.map(p=>`p${p.page}: ${p.fill}% ${p.title||'continued lesson'}`).join('\n'));
