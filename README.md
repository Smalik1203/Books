...
| **page box, − / level / + / fit** | Chrome's PDF toolbar, in Chrome's order. The level is typed — any percentage from 10 to 500, since a proof gets read at whatever makes one figure legible. The fit button shows the mode it is in — an upright sheet for fit to page, a wide one for fit to width — and returns to fit to page from any fixed percentage |
| the keyboard | `↑` `↓` scroll, `←` `→` turn the page — or scroll sideways while there is anywhere sideways to go, turning the page at the far edge. Space and shift-space take a screenful, `PageUp`/`PageDown`/`Home`/`End` whole pages, `ctrl` with `+` `-` `0` works the zoom. All of it works in every mode, and whether the focus is on the studio or on the book |
