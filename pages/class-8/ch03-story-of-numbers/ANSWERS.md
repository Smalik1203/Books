...
The Egyptian symbols, as the chapter names them: stroke $1$, arch $10$,
coil $100$, lotus $1000$, finger $10^4$, tadpole $10^5$, kneeling figure
$10^6$.
...
### Think and Reflect (the Chinese rods)

With uprights in every place, $41$ is four uprights, a space, and one
upright. If the space is lost or misjudged it can read as **five uprights,
that is $5$**; if the space is taken for an empty place it reads as
**$401$**. A place value system needs **a symbol for an empty place, used
everywhere, so that every place is marked**.
