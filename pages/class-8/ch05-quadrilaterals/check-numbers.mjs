...
const S1 = '### Exercise Set 5.1', S2 = '### Exercise Set 5.2', S3 = '### Exercise Set 5.3';
const S4 = '### Exercise Set 5.4', S5 = '### Exercise Set 5.5', S6 = '### Exercise Set 5.6';
const R = ['i', 'ii', 'iii', 'iv'];

// 5.1
mentions('5.1 Q2', mdItem(S1, 2), 7, 14);
[6, 8, 10, 12].forEach((n, k) => mentions(`5.1 Q4 (${R[k]})`, mdPart(S1, 4, R[k]), diagonals(n)));
mentions('5.1 Q5', mdItem(S1, 5), fromDiagonals(35));
is('5.1 Q10: n(n-3) always even', [...Array(200)].every((_, n) => (n * (n - 3)) % 2 === 0));

// 5.2
[5, 8, 15, 100].forEach((n, k) => mentions(`5.2 Q1 (${R[k]})`, mdPart(S2, 1, R[k]), angleSum(n)));
[[70, 95, 105], [60, 60, 120], [90, 90, 58]].forEach((a, k) =>
  mentions(`5.2 Q2 (${R[k]})`, mdPart(S2, 2, R[k]), 360 - a.reduce((x, y) => x + y)));
[900, 1620, 2340].forEach((s, k) => mentions(`5.2 Q3 (${R[k]})`, mdPart(S2, 3, R[k]), fromSum(s)));
ok('5.2 Q3 (iv): 1000 fits no polygon', fromSum(1000), null);
is('5.2 Q3 (iv) says no polygon', /no polygon/.test(mdPart(S2, 3, 'iv')));
[8, 9, 12, 20].forEach((n, k) => mentions(`5.2 Q4 (${R[k]})`, mdPart(S2, 4, R[k]), interior(n)));
mentions('5.2 Q5', mdItem(S2, 5), ...[1, 2, 3, 4].map(k => k * 360 / 10));
mentions('5.2 Q6', mdItem(S2, 6), (360 - 120) / 3);
mentions('5.2 Q7', mdItem(S2, 7), fromInterior(150));
ok('5.2 Q8: 100, 100, 100 leaves', 360 - 300, 60);
mentions('5.2 Q10', mdItem(S2, 10), 144, 10, 11);
ok('5.2 Q10: 145 fits no regular polygon', fromInterior(145), null);
{ // nearest regular angles either side of 145
  ...
  ok('5.2 Q10: nearest below and above', [below, above], [interior(10), interior(11)]);
  ok('5.2 Q10: 147 3/11 and 32 8/11', [interior(11), exterior(11)], [147 + 3 / 11, 32 + 8 / 11]);
  is('5.2 Q10 prints 147 3/11', /147\\tfrac\{3\}\{11\}/.test(mdSet(S2)));
}

// 5.3
[4, 6, 10, 18].forEach((n, k) => mentions(`5.3 Q1 (${R[k]})`, mdPart(S3, 1, R[k]), exterior(n)));
[45, 30, 18, 5].forEach((e, k) => mentions(`5.3 Q2 (${R[k]})`, mdPart(S3, 2, R[k]), fromExterior(e)));
{ const ext = [65, 80, 70, 55]; const fifth = 360 - ext.reduce((a, b) => a + b);
  mentions('5.3 Q3', mdItem(S3, 3), fifth);
  // the answer list itself, in order — the check line after it repeats the numbers
  const list = (mdSet(S3).match(/\*\*(\$115[\s\S]*?)\*\*/) || [])[1] || '';
  ok('5.3 Q3: the five interior angles', (text(list).match(/\d+/g) || []).map(Number), [...ext, fifth].map(e => 180 - e)); }
mentions('5.3 Q4', mdItem(S3, 4), fromInterior(162));
ok('5.3 Q5: 7 and 130 fit nothing', [fromExterior(7), fromInterior(130)], [null, null]);
ok('5.3 Q6: octagon interior = 3 x exterior', interior(8), 3 * exterior(8));
is('5.3 Q6: only the octagon', [...Array(300)].map((_, i) => i + 3).filter(n => near(interior(n), 3 * exterior(n))).join() === '8');
mentions('5.3 Q7', mdItem(S3, 7), [...Array(300)].map((_, i) => i + 3).find(n => near(interior(n), 5 * exterior(n))));
mentions('5.3 Q8', mdItem(S3, 8), fromSum(2160));
ok('5.3 Q8: the exterior route', (2160 + 360) / 180, fromSum(2160));
ok('5.3 Q10: regular polygons that fill a point', [...Array(300)].map((_, i) => i + 3).filter(n => near(360 / interior(n), Math.round(360 / interior(n)))), [3, 4, 6]);

// 5.4
mentions('5.4 Q7', mdItem(S4, 7), 180 - 72);

// 5.5
[50, 90, 128, 37].forEach((a, k) => {
  const after = mdPart(S5, 1, R[k]).split(':').slice(1).join(':');
  ok(`5.5 Q1 (${R[k]}): the other three`, (after.match(/\d+/g) || []).map(Number), [180 - a, a, 180 - a]);
});
mentions('5.5 Q2', mdItem(S5, 2), 9, 5, 2 * (9 + 5));
mentions('5.5 Q3', mdItem(S5, 3), 2 * 180 / 5, 3 * 180 / 5);
mentions('5.5 Q4', mdItem(S5, 4), 2 * 7, 2 * 4);
{ const y = lin(2, 1, 1, 8); mentions('5.5 Q5', mdItem(S5, 5), y, 2 * y + 1); }
{ const a = (48 / 2 - 3) / 2; mentions('5.5 Q6', mdItem(S5, 6), a, a + 3); }

// 5.6
{ const s = hyp(5, 12); mentions('5.6 Q2', mdItem(S6, 2), s, 4 * s); }
mentions('5.6 Q3', mdItem(S6, 3), hyp(8, 15));
mentions('5.6 Q4', mdItem(S6, 4), 2 * leg(13, 10 / 2));
mentions('5.6 Q5', mdItem(S6, 5), 90, 45);
mentions('5.6 Q9', mdItem(S6, 9), 10);
{ // diagonals 6 and 8 at right angles, the 8 cut 4 + 4 and the 6 cut 2 + 4: sides
  const sides = [hyp(4, 2), hyp(4, 2), hyp(4, 4), hyp(4, 4)];
  ok('5.6 Q7: a kite, and not a rhombus', [sides[0] === sides[1] && sides[2] === sides[3], new Set(sides).size === 1], [true, false]);
  is('5.6 Q7 prints the instance', /6 cm and\s+8 cm/.test(mdItem(S6, 7)) && /2 cm and 4 cm/.test(mdItem(S6, 7))); }
